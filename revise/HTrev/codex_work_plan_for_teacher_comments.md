...
- [ ] Reviewer 1 comment 2 の回答文を丁寧な表現に置換する。
  - 対応コメント: Comment 4
  - 置換案:
...
- [ ] Reviewer 2 冒頭も Reviewer 1 と同様の始め方にする。
  - 対応コメント: Comment 5
...
- [ ] Reviewer 2 comment 3 の Vinogradova 関連表現を修正する。
...
- [ ] Reviewer 2 comment 4 の f* 回答を教員案に置換する。
...
- [ ] Reviewer 2 comment 5 の Fig. 2 caption 対応に `[for the fit]` を明記する。
  - 対応コメント: Comment 15
...
- [ ] Reviewer 2 comment 8 付近の本文・回答表現を `on the slippery surface` に合わせる。
  - 対応コメント: Comment 17

- [ ] Reviewer 2 comment 9 の回答を教員案に置換する。
...
- [ ] Reviewer 2 comment 10 の回答冒頭を `We apologize for the confusion.` に変更し、参照図を `Fig. S3` に合わせる。
  - 対応コメント: Comment 20, Comment 21

- [ ] Reviewer 2 comment 11 の表現を修正する。
...
- [ ] Reviewer 2 comment 12 の回答を教員案に置換する。
  - 対応コメント: Comment 24
  - ただし `common` の意味が特に不要なら削る、または明確な語に置換する。
  - 対応コメント: Comment 25

- [ ] Reviewer 2 comment 13 の回答を教員案に置換する。
  - 対応コメント: Comment 28
  - `unpatterned reference substrates` を response と SI の両方で使う。
  - 関連する短い注釈 `unpatterned` も反映する。
  - 対応コメント: Comment 29

- [ ] Reviewer 2 comment 14 または関連箇所に、`leaving the intrinsic slip length on hydrophobic surfaces unresolved` を必要に応じて反映する。
  - 対応コメント: Comment 30

- [ ] Reviewer 2 comment 15 または関連箇所の末尾に、`. as discussed in our response to question 9.` の趣旨を反映する。
  - 対応コメント: Comment 31

### Reviewer 3 と Non-scientific changes

- [ ] Reviewer 3 への回答の後に `Non-scientific changes` の章を追加する。
  - 対応コメント: Comment 1
...
- [ ] CNT 関連本文を確認し、`CNT slip length` など不自然な表現を `slip length in CNTs` 等へ修正する。
  - 対応コメント: Comment 7

- [ ] Eq. 4 導出の本文説明を、Supporting Information I への参照つきで修正する。
  - 対応コメント: Comment 9, Comment 11

- [ ] f* の式番号参照・本文説明を response と整合させる。
  - 対応コメント: Comment 12, Comment 14

- [ ] `on the slippery surface` に合わせるべき本文表現を修正する。
  - 対応コメント: Comment 17

- [ ] Teflon/HOPG の議論を教員案と整合させる。
  - 対応コメント: Comment 18, Comment 19
  - 既存の議論が更新済みか、response の line 197-217 と整合するか確認する。

- [ ] 120° / CF3 に関する本文修正を教員案に合わせる。
...
- [ ] Contact angle 測定説明を `unpatterned reference substrates` を用いる表現に変更する。
  - 対応コメント: Comment 28, Comment 29

- [ ] Hydrophobic surfaces の consensus に関する記述に、`leaving the intrinsic slip length on hydrophobic surfaces unresolved` の趣旨を必要に応じて反映する。
  - 対応コメント: Comment 30
...
- [ ] Fig. S3 caption 内のアルファベットの組み合わせを修正する。
  - 対応コメント: Comment 1

- [ ] Table S2 caption の `calculated from` を `predicted by` に変更する。
  - 対応コメント: Comment 23

- [ ] Table S2 の列見出しを `b predicted by C = xx nm (nm)` 形式に変更する。
  - 対応コメント: Comment 23

- [ ] Contact angle 測定説明を `unpatterned reference substrates` に変更する。
  - 対応コメント: Comment 28, Comment 29

## 4. 確認・ビルド

- [ ] 修正後、`latexmk -pdf main.tex` を実行して main PDF がビルドできるか確認する。
- [ ] 修正後、`latexmk -pdf supporting_information.tex` を実行して SI PDF がビルドできるか確認する。
- [ ] 修正後、`latexmk -pdf point_by_point_response.tex` を実行して response PDF がビルドできるか確認する。
- [ ] `rg` で以下を確認する。
