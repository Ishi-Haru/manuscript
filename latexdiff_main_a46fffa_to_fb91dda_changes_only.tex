...
%DIF DEL C:\Users\haruy\AppData\Local\Temp\diff-tex-l3rd_tnr\old\main.tex   Sat Apr 25 13:46:54 2026
%DIF ADD C:\Users\haruy\AppData\Local\Temp\diff-tex-l3rd_tnr\new\main.tex   Sun Apr 26 17:13:14 2026
...
%DIF -------
\title[title]{Water Need Not Slip on Hydrophobic Surfaces: Insights from Slip Length Mapping} %DIF > 
%DIF -------
...
\newcommand{\DIFaddincludegraphics}[2][]{{\color{blue}\DIFOincludegraphics[#1]{#2}}} %DIF PREAMBLE
...
Previous studies have suggested that solid-liquid interfacial properties such as wettability \cite{Huang2008-xu}, surface roughness \cite{Vinogradova2006-cw}, surface charge \cite{Xie2020-as}, surface free energy distribution \cite{Tocci2014-es}, and surface nanobubbles \cite{Li2016-ly} strongly influence the slip length. However, despite extensive efforts, their quantitative relationships remain unclear. In particular, compared with hydrophilic surfaces (i.e., contact angles \(< \SI{90}{degree}\)), there is no clear consensus on slip lengths for hydrophobic surfaces (i.e., contact angles \(> \SI{90}{degree}\)). \DIFaddbegin \DIFadd{For hydrophilic smooth surfaces, reported slip lengths are generally close to zero within experimental uncertainty, whereas for hydrophobic surfaces the reported values scatter widely from near-zero to tens or even hundreds of nanometers, as summarized in Fig.~\ref{fgr:previous_slip_length}. Previous studies have attributed such large apparent slip to different origins, including surface roughness \mbox{%DIFAUXCMD
\cite{Vinogradova2006-cw, Lund2012-cr}}\hskip0pt%DIFAUXCMD
, nanobubbles \mbox{%DIFAUXCMD
\cite{Li2016-ly}}\hskip0pt%DIFAUXCMD
, and measurement artifacts \mbox{%DIFAUXCMD
\cite{Pradhan2025-yl}}\hskip0pt%DIFAUXCMD
, leaving the intrinsic slip length on hydrophobic surfaces unresolved. }\DIFaddend A scaling law relating the slip length \(b\) to the intrinsic contact angle \(\theta\) is derived as follows \cite{Huang2008-xu}:
\begin{equation}
  b = \frac{C}{(1 + \cos \theta)^{2}}. \label{eqn:slip length scaling}
\end{equation}
where \(C\) is a \DIFdelbegin \DIFdel{proportionality constant}\DIFdelend \DIFaddbegin \DIFadd{prefactor with dimensions of length, and \(b\) is reported in nanometers in this work}\DIFaddend . Slip lengths obtained from molecular dynamics (MD) simulations are well fit by Eq.~\ref{eqn:slip length scaling} with \DIFdelbegin \DIFdel{\(C = 0.41\)}\DIFdelend \DIFaddbegin \DIFadd{\(C = \SI{0.41}{\nano\metre}\)}\DIFaddend , indicating almost no slip even at intrinsic contact angles of \SI{120}{\degree}\DIFdelbegin \DIFdel{, which is the near-maximum achievable on a topographically smooth flat surface \mbox{%DIFAUXCMD
\cite{Nishino1999-dk}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{. We use \(\SI{120}{\degree}\) as a representative upper-range value for chemically homogeneous smooth flat surfaces because low-surface-energy fluorinated surfaces, including PTFE and densely packed \(-\mathrm{CF}_3\)-terminated surfaces, typically approach this value; much larger apparent contact angles generally require micro/nanoscale roughness and a Cassie state rather than surface chemistry alone \mbox{%DIFAUXCMD
\cite{Nishino1999-dk, Quere2008-lq, Verho2011-as}}\hskip0pt%DIFAUXCMD
}\DIFaddend . In contrast, when experimental slip length data from various studies are compiled (Fig.~\ref{fgr:previous_slip_length}), they show significant deviation from the MD-predicted scaling law with \DIFdelbegin \DIFdel{\(C = 0.41\)}\DIFdelend \DIFaddbegin \DIFadd{\(C = \SI{0.41}{\nano\metre}\)}\DIFaddend . For example, Wu et al. compiled 11 experimental studies and reported \DIFdelbegin \DIFdel{$C=6.0$}\DIFdelend \DIFaddbegin \DIFadd{\(C = \SI{6.0}{\nano\metre}\)}\DIFaddend , corresponding to slip lengths of tens of nanometers above \SI{90}{\degree} \cite{Wu2016-vv}. We additionally incorporated 14 experimental studies into Wu et al.'s plot. The data deviated from the \DIFdelbegin \DIFdel{$C=0.41$ }\DIFdelend \DIFaddbegin \DIFadd{\(C = \SI{0.41}{\nano\metre}\) }\DIFaddend curve and even from the \DIFdelbegin \DIFdel{$C=6.0$ }\DIFdelend \DIFaddbegin \DIFadd{\(C = \SI{6.0}{\nano\metre}\) }\DIFaddend curve, particularly on hydrophobic surfaces. Moreover, although not included in Fig.~\ref{fgr:previous_slip_length}, a much larger value ($\SI{\sim1}{\micro\metre}$) has been measured \cite{Tretheway2002-et}.
...
\end{equation}
where \(\gamma_{\text{tip}}\) is the viscous damping coefficient acting on the probe tip, \DIFaddbegin \DIFadd{\(\dot{h}\) is the probe-substrate approach velocity, }\DIFaddend \(\eta\) is the viscosity of the surrounding medium, \(R\) is the radius of curvature of the probe tip, \(h\) is the probe-substrate distance, \(b_t\) and \(b_s\) are the slip lengths at the probe tip and substrate surfaces, respectively, and \(f^*\) is a correction factor representing the reduction of viscous drag due to slip \cite{Vinogradova1995-ct}. \DIFaddbegin \DIFadd{Equation~\ref{eqn:hydrodynamic force} is the lubrication-theory expression for the hydrodynamic force between a sphere and a plane with slip boundary conditions \mbox{%DIFAUXCMD
\cite{Vinogradova1995-ct}}\hskip0pt%DIFAUXCMD
. Experimentally, \(h(t)\) is obtained from the measured probe-substrate distance trajectory, and \(\dot{h}\) is calculated numerically from this \(h(t)\) data.
