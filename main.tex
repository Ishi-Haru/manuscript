...
\renewcommand*\acs@keywords@print{%
  \ifx\@keywords\@empty\else
    ...
    \ChangeTagStart{non_sci_keywords_color}{\color{blue}\@keywords}\ChangeTagEnd{non_sci_keywords_color}
    \par\vspace{1em}
  \fi
}
\makeatother
...
\ChangeTagStart{r2_2_secchi_cnt_comparison}The large slip length observed on planar HOPG should also be distinguished from the massive, radius-dependent water slippage reported inside carbon nanotubes (CNTs). Secchi et al. reported exceptionally large slip lengths in CNTs under cylindrical nanoscale confinement and noted that slip lengths inside CNTs are consistently much larger than those measured on planar hydrophobic and graphite surfaces, where \(b\) is typically at most a few tens of nanometers\cite{Secchi2016-ks}. Thus, the CNT results are not inconsistent with our conclusion for nanoscopically flat planar surfaces.\ChangeTagEnd{r2_2_secchi_cnt_comparison}
