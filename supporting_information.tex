...
\title[Title_supporting_information]
  ...
  Hydrophobicity Does Not Determine Water Slip Length: Insights from Slip Length Mapping}
...
\ChangeTagStart{r2_4_fstar_function_si}We compared the sensitivity of slip length measurements between conventional contact-mode AFM and the proposed FM-AFM method. In contact-mode AFM, the cantilever deflection \(x\) is measured as a sensor voltage, and the relationship between the deflection sensor voltage \(V_x\) and \(x\) is expressed using the sensitivity \(s\) as \(x = sV_x\). Furthermore, to determine the viscous drag in Eq.~\ref{eqn:hydrodynamic force} from the restoring force of the cantilever, \(kx\) (\(k\) is the spring constant), the following relation holds:
\begin{equation}
  F = -ksV_x = -\frac{6\pi\eta R^2 \dot{h}}{h} f^{*}(h, b_t, b_s).
  \tag{S7}
  \label{eqn:s7}
\end{equation}
...
\begin{equation}
  \frac{6\pi\eta R^2}{h} f^{*}(h, b_t, b_s) = \gamma_{\text{bulk}}\left(\frac{V_{A,\text{bulk}}}{V_A(h)} - 1\right).
  \tag{S8}
  \label{eqn:s8}
\end{equation}
\ChangeTagEnd{r2_4_fstar_function_si}Here, the measurement sensitivity is defined as the change in signal voltage with respect to the substrate slip length, \(\partial V/\partial b_s\). Differentiating Eqs.~\ref{eqn:s7} and \ref{eqn:s8} yields the sensitivities for each method, \(S_{\text{contact}}\) and \(S_{\text{FM}}\):
\begin{equation}
...
\ChangeTagStart{r2_6_tip_calibration_fit_si}\ChangeTagStart{r2_7_nanobubble_fit_si}
\begin{figure}[p]
  \centering
  \includegraphics[width=1.0\linewidth]{figs/DLC_bubble.png}
  \caption{
    Representative damping-coefficient fitting curves for (a) an atmospheric-plasma-treated DLC substrate and (b) a location above a nanobubble.
...
\end{figure}
\ChangeTagEnd{r2_7_nanobubble_fit_si}\ChangeTagEnd{r2_6_tip_calibration_fit_si}

\ChangeTagStart{non_sci_figs3_caption}
\begin{figure}[p]
  \centering
...
\end{figure}
\ChangeTagEnd{non_sci_figs3_caption}

\ChangeTagStart{r2_11_units_comparison_table_si}
\begin{table}[p]
  \caption{Measured slip lengths and values predicted by Eq.~\ref{eqn:slip length scaling} for the substrates investigated in this study.}
  \label{tbl:contact_angle_slip}
  \centering
  \resizebox{\textwidth}{!}{%
  \begin{tabular}{lcccc}
    \hline
    Substrate & Contact Angle (\si{\degree}) & \(b\) predicted by \(C = 0.41~\mathrm{nm}\) (nm) & \(b\) predicted by \(C = 6.0~\mathrm{nm}\) (nm) & Measured \(b\) (nm) \\
    \hline
    Mica & \(< 3\) & \(\sim 0.10\) & \(\sim 1.50\) & \(-1.7 \pm 0.3\) \\
    Oxygen plasma-treated silica & \(28.1\) & \(0.12\) & \(1.69\) & \(-0.7 \pm 0.6\) \\
    FDTS & \(97.9\) & \(0.55\) & \(8.06\) & \(-1.7 \pm 0.2\) \\
    FOPA & \(105.0\) & \(0.75\) & \(10.92\) & \(-0.9 \pm 0.6\) \\
    Teflon & \(111.1\) & \(1.00\) & \(14.65\) & \(0.8 \pm 0.8\) \\
    HOPG (DI water) & \(59.0\) & \(0.18\) & \(2.61\) & \(43.2 \pm 5.8\) \\
    HOPG (\(10\) mM KCl aq.) & \(62.8\) & \(0.19\) & \(2.83\) & \(-0.1 \pm 0.7\) \\
    HOPG (\(10\) mM NaCl aq.) & \(57.0\) & \(0.17\) & \(2.51\) & \(-2.3 \pm 0.6\) \\
    \hline
  \end{tabular}
  }
\end{table}
\ChangeTagEnd{r2_11_units_comparison_table_si}
