...
%DIF DEL C:\Users\haruy\AppData\Local\Temp\diff-tex-f23c4m1b\old\supporting_information.tex   Sat Apr 25 13:46:54 2026
%DIF ADD C:\Users\haruy\AppData\Local\Temp\diff-tex-f23c4m1b\new\supporting_information.tex   Sun Apr 26 17:13:14 2026
...
%DIF 111c111
%DIF <   Water Does Not Slip on Hydrophobic Surfaces: Insights from Slip Length Mapping}
%DIF -------
  Water Need Not Slip on Hydrophobic Surfaces: Insights from Slip Length Mapping} %DIF > 
%DIF -------
...
\begin{equation}
  ...
  \label{eqn:s8}
\end{equation}
Here, the measurement sensitivity is defined as the change in signal voltage with respect to the substrate slip length, \(\partial V/\partial b_s\). Differentiating Eqs.~\ref{eqn:s7} and \ref{eqn:s8} yields the sensitivities for each method, \(S_{\text{contact}}\) and \(S_{\text{FM}}\):
\begin{equation}
...
\begin{figure}[p]
  \centering
  \DIFaddbeginFL \includegraphics[width=1.0\linewidth]{figs/DLC_bubble.png}
  \caption{
    \DIFaddFL{Representative damping-coefficient fitting curves for (a) tip slip-length calibration on an atmospheric-plasma-treated DLC substrate and (b) slip-length extraction directly above a nanobubble. The mean absolute errors were \mbox{%DIFAUXCMD
\SI{0.016}{\micro\newton\second\per\metre} }\hskip0pt%DIFAUXCMD
and \mbox{%DIFAUXCMD
\SI{0.014}{\micro\newton\second\per\metre} }\hskip0pt%DIFAUXCMD
...
\begin{table}[p]
  \caption{\DIFdelbeginFL \DIFdelFL{Measured contact angles and }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Comparison of measured }\DIFaddendFL slip lengths \DIFdelbeginFL \DIFdelFL{of }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{with values calculated from Eq.~\ref{eqn:slip length scaling} for }\DIFaddendFL the substrates investigated in this study\DIFaddbeginFL \DIFaddFL{. The mean absolute errors between the measured values and the calculated values over all entries are \mbox{%DIFAUXCMD
\SI{6.56}{\nano\metre} }\hskip0pt%DIFAUXCMD
for \(C = \SI{0.41}{\nano\metre}\) and \mbox{%DIFAUXCMD
\SI{11.17}{\nano\metre} }\hskip0pt%DIFAUXCMD
for \(C = \SI{6.0}{\nano\metre}\).}\DIFaddendFL }
...
\SI{28.1}{\degree} }\hskip0pt%DIFAUXCMD
}%DIFDELCMD < & %%%
\DIFdelFL{\(-0.7 \pm \SI{0.6}{\nano\metre}\) }%DIFDELCMD < \\
%DIFDELCMD <     %%%
\DIFdelFL{FDTS }%DIFDELCMD < & %%%
\DIFdelFL{\mbox{%DIFAUXCMD
\SI{97.9}{\degree} }\hskip0pt%DIFAUXCMD
}%DIFDELCMD < & %%%
\DIFdelFL{\(-1.7 \pm \SI{0.2}{\nano\metre}\) }%DIFDELCMD < \\
%DIFDELCMD <     %%%
\DIFdelFL{FOPA }%DIFDELCMD < & %%%
\DIFdelFL{\mbox{%DIFAUXCMD
\SI{105.0}{\degree} }\hskip0pt%DIFAUXCMD
}%DIFDELCMD < & %%%
\DIFdelFL{\(-0.9 \pm \SI{0.6}{\nano\metre}\) }%DIFDELCMD < \\
%DIFDELCMD <     %%%
\DIFdelFL{Teflon }%DIFDELCMD < & %%%
\DIFdelFL{\mbox{%DIFAUXCMD
\SI{111.1}{\degree} }\hskip0pt%DIFAUXCMD
}%DIFDELCMD < & %%%
\DIFdelFL{\(0.8 \pm \SI{0.8}{\nano\metre}\) }%DIFDELCMD < \\
%DIFDELCMD <     %%%
\DIFdelFL{HOPG (DI water) }%DIFDELCMD < & %%%
\DIFdelFL{\mbox{%DIFAUXCMD
\SI{59.0}{\degree} }\hskip0pt%DIFAUXCMD
}%DIFDELCMD < & %%%
\DIFdelFL{\(43.2 \pm \SI{5.8}{\nano\metre}\) }%DIFDELCMD < \\
%DIFDELCMD <     %%%
\DIFdelFL{HOPG (\mbox{%DIFAUXCMD
\SI{10}{\milli\mole\per\litre} }\hskip0pt%DIFAUXCMD
KCl aq.) }%DIFDELCMD < & %%%
\DIFdelFL{\mbox{%DIFAUXCMD
\SI{62.8}{\degree} }\hskip0pt%DIFAUXCMD
}%DIFDELCMD < & %%%
...
NaCl aq.) }%DIFDELCMD < & %%%
\DIFdelFL{\mbox{%DIFAUXCMD
\SI{57.0}{\degree} }\hskip0pt%DIFAUXCMD
}%DIFDELCMD < & %%%
