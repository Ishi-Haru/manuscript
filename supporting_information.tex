...
\begin{figure}[p]
  ...
  \caption{
    Representative damping-coefficient fitting curves for (a) tip slip-length calibration on an atmospheric-plasma-treated DLC substrate and (b) slip-length extraction directly above a nanobubble. The mean absolute errors were \SI{0.016}{\micro\newton\second\per\metre} and \SI{0.014}{\micro\newton\second\per\metre} for (a) and (b), respectively.
  }
...
\begin{table}[p]
  \caption{Comparison of measured slip lengths with values calculated from Eq.~\ref{eqn:slip length scaling} for the substrates investigated in this study. The mean absolute errors between the measured values and the calculated values over all entries are \SI{6.56}{\nano\metre} for \(C = \SI{0.41}{\nano\metre}\) and \SI{11.17}{\nano\metre} for \(C = \SI{6.0}{\nano\metre}\).}
